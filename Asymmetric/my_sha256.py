...
class SHA256:
    ...
    @staticmethod
    def _compression_function(w, h, k):
        a, b, c, d, e, f, g, hh = h
    ...
    def calculate_hash(self, data):
        ...
        chunks = self._create_block(data)
        for chunk in chunks:
            w = self._extend_block(chunk)
            h = self._compression_function(w, h, k)
        hh = (h[0] << 224) | (h[1] << 192) | (h[2] << 160) | (h[3] << 128) | (h[4] << 96) | (h[5] << 64) | (
                h[6] << 32) | h[7]
        return format(hh, '064x')
