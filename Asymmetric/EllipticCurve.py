...
if __name__ == '__main__':
    ...
    # Приватный ключ (примерно выбираем для простоты)
    private_key = 55441196065363246126355624130324183196576709222340016572108097750006097525544

    # Формирование подписи
    signature_instance = Digital_signature()
    signature_instance.message_hash = hashed_message
    signature = signature_instance.form(private_key)
    print(f"Generated signature: {signature}")

    # Публичный ключ, соответствующий указанному приватному ключу
    public_key = curve.multiplyPoint(private_key, curve.g)
    print(f"Public key: {public_key}")

    # Проверка подписи
    verification_result = signature_instance.check(public_key, signature)
